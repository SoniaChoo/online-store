...
func InsertDish(d *model.Dish) error {
	...
	defer cancel()
	_, err = db.QueryContext(ctx, "insert into dish(rest_id, price, dish_name, description, stock, sales, favorite) values(?,?,?,?,?,?,?)",
		d.RestId, d.Price, d.DishName, d.Description, d.Stock, d.Sales, d.Favorite)
	if err != nil {
